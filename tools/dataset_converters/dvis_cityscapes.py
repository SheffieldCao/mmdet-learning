...
def collect_files(img_dir, gt_dir):
    ...
    for img_file in glob.glob(osp.join(img_dir, '*.png')):
        ...
        # Depth gt annotations
        depth_file = gt_dir + img_file[
            len(img_dir):-len(suffix)] + 'gtFine_depth.png'
        files.append((img_file, inst_file, segm_file, depth_file))
...
def load_img_info(files):
    img_file, inst_file, segm_file, depth_file = files
    ...
    img_info = dict(
        ...
        depth_file=osp.basename(depth_file)
        )
...
def parse_args():
    ...
    parser.add_argument('--img-dir', default='leftImg8bitDVIS', type=str)
    parser.add_argument('--gt-dir', default='gtFineDVIS', type=str)
